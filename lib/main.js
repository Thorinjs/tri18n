...
function transform(code, _default) {
  ...
  var text = util.innerKey(CONFIG.data, code),
      defaultText = typeof _default === 'string' ? _default : code,
      vars = (typeof _default === 'undefined' ? 'undefined' : _typeof(_default)) === 'object' && _default ? _default : {};
  if (text == null) {
    if (!MISSING_TRANSLATIONS[code]) {
      if (CONFIG.debug) {
        console.debug('tri18n: ' + code + ' has no translation');
      }
...
function configure(opt) {
  if (_typeof(opt.data) === 'object' && opt.data) {
...
function data(dataObj) {
  if ((typeof dataObj === 'undefined' ? 'undefined' : _typeof(dataObj)) === 'object' && dataObj) {
...
function load(onDone) {
  if (_typeof(CONFIG.data) === 'object' && CONFIG.data) return onDone && onDone();
  // Check if we have anything in cache.
  if (CONFIG.cache) {
    var oldLang = util.loadCachedLanguage(CONFIG.lang);
  ...
  util.download(function (e, langObj) {
 ...
 * */
function setLanguage(langCode, done) {
